...
def DepthMapfromStereoImages():
    ...
    # 
    cv.namedWindow('img', 0)
    cv.imshow('img',(disparity-min_disp)/num_disp)
    cv.waitKey(0)
...
def EpipolarGeometry(pts1, pts2, F, maskF, FT, maskE):
    ...
    cv.namedWindow('Fleft', 0)
    cv.imshow('Fleft', img3)
    cv.namedWindow('Fleft2', 0)
    cv.imshow('Fleft2', img4)

    cv.namedWindow('Fright', 0)
    cv.imshow('Fright', img5)
    cv.namedWindow('Fright2', 0)
